...
def test_run_data_update_formats_update_pool(monkeypatch, tmp_path):
    ...
    config = {
        ...
        "data_update": {
            ...
            "command": [
                ...
                "--end-date",
                "{data_update_end_date}",
                "--work-dir",
                "{data_update_work_dir}",
                "--max-workers",
                "{data_update_max_workers}",
                "--source-workers",
                "{data_update_source_workers}",
                "--delay",
                "{data_update_delay}",
                "--retry-count",
                "{data_update_retry_count}",
                "--source-consecutive-failures",
                "{data_update_source_consecutive_failures}",
                "--source-cooldown",
                "{data_update_source_cooldown}",
                "--max-source-cooldown",
                "{data_update_max_source_cooldown}",
                "--source-min-attempts",
                "{data_update_source_min_attempts}",
                "--source-error-rate",
                "{data_update_source_error_rate}",
                "--source-disable-after-cooldowns",
                "{data_update_source_disable_after_cooldowns}",
            ],
        },
    ...
    assert calls["command"] == [
        ...
        "pool",
        "--sources",
        "baostock,yahoo,akshare",
        "--instruments",
        "SH600000,SZ000001",
        "--end-date",
        "today",
        "--work-dir",
        "~/.qlib/stock_data/cn_daily_update",
        "--max-workers",
        "6",
        "--source-workers",
        "baostock=1,yahoo=3,akshare=2",
        "--delay",
        "0",
        "--retry-count",
        "4",
