...
适用于：使用第三方打包好的二进制目录（结构与官方一致）。

示例（来自 `scripts/README.md` 思路）：

```bash
# 下载 release 中的 qlib_bin.tar.gz 后解压到 cn_data 根目录
mkdir -p ~/.qlib/qlib_data/cn_data
tar -zxvf qlib_bin.tar.gz -C ~/.qlib/qlib_data/cn_data --strip-components=2
```

解压后请确认根目录下出现 `features/`、`calendars/`、`instruments/` 等。不同发布包 `strip-components` 层级可能不同，需按实际压缩包顶层路径调整。
