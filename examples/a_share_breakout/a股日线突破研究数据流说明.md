...
| 阶段 6.1 | 已完成换手控制诊断 | `n_drop=1`、`hold_thresh=5` 显著降低换手和成本拖累，但成本后超额仍为负。 |
...
1. 没有训练任何机器学习模型。
2. 阶段 6 已形成 60/120 日、open/vwap、低/中/高成本完整回测矩阵，四组中性成本 baseline 成本后超额年化均明显为负。
3. 阶段 6.1 已确认降低强制换手能显著改善结果，但不足以让信号转正。
4. 还没有实现阶段 7 的事件生命周期策略；按阶段 6/6.1 结果，应先修正规则、候选过滤与执行价问题，再决定是否进入阶段 7。
...
| baseline | 成本后超额年化 | 日均换手 | 持有天数代理 |
|---|---:|---:|---:|
| `baseline_60d_open` | -64.64% | 3.80% | 26.35 |
| `baseline_60d_vwap` | -51.71% | 2.92% | 34.27 |
| `baseline_120d_open` | -61.43% | 5.71% | 17.51 |
...
6. 阶段 6.1 已确认换手控制能大幅改善但不能单独解决信号问题。
7. Qlib 提供通用框架，本目录提供 A 股突破研究的具体定义、过滤、标签和分数。
